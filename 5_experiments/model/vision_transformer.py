...
from model._builder import build_model_with_cfg
from model._manipulate import named_apply, checkpoint_seq, adapt_input_conv
from model._registry import generate_default_cfgs, register_model, register_model_deprecations
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        attn_output, attn_weights = self.attn(self.norm1(x))  # Lấy output và attention     weights
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor, return_attention: bool = False):
        ...
        attentions = []  # Danh sách để lưu attention weights

        # Forward qua các block và thu thập attention weights
        for blk in self.blocks:
            if return_attention:
                x, attn_weights = blk(x)  # Sửa lớp Block để trả về attn_weights
